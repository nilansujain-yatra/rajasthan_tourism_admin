...
export async function GET(request: NextRequest) {
  ...
  try {
    const url = new URL(`${getBaseApiUrl()}/boardingV2/getAllBoardingPass_V2`)
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : 'Unable to fetch generated boarding passes.' },
      { status: 500 },
    )
  }
}
