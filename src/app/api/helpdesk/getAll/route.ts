...
function getHelpdeskUrl(request: NextRequest) {
  const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL?.replace(/\/+$/, '')
    ?? 'https://api-tourist.rajasthan.gov.in/rajasthan/api/v1'
  const url = new URL(`${baseUrl}/helpdesk/getAll`)
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(getHelpdeskUrl(request), {
      method: 'GET',
      headers: {
    ...
    const message = error instanceof Error ? error.message : 'Unable to fetch helpdesk tickets.'
