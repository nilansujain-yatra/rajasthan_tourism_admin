...
function buildServerLogsUrl(request: NextRequest) {
  const url = new URL(`${getBaseApiUrl()}/reports/userServerLogsReport`)
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(buildServerLogsUrl(request), {
      method: 'GET',
      headers: {
    ...
    const message = error instanceof Error ? error.message : 'Unable to fetch server logs.'
