...
function buildUpstreamUrl(request: NextRequest) {
  ...
  const url = new URL(`${baseUrl}/inventory/reports/summary/headReport/v2`)
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to fetch head summary report.'
